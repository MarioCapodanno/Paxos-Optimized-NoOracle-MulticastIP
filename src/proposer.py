...
class Proposer:
    def __init__(self, config, id):
        ...
        # The idea is to check if the number ofpromises with a ballot number higher than the current one are 
        # reaching the quorum, if so, we retry with a higher ballot number.
    ...
    def start_phase1(self):
        ...
        logging.debug(f"Proposer {self.id}: starting Phase 1 with ballot {self.current_ballot}")
    ...
    def handle_promise(self, msg):
        """Handle the received PROMISE message from acceptor.
            The 'msg' object has fields: (
                ...
                acceptor_id)
        """
        _, ballot, proposer_id, accepted_ballot, accepted_pid, accepted_value, acceptor_id = msg
        ...
        if self.phase != "PREPARE" or ballot != self.current_ballot or proposer_id != self.id:
            logging.debug(f"Proposer {self.id}: ignoring PROMISE (not current proposal)")
            return
        
        logging.debug(f"Proposer {self.id}: received PROMISE from acceptor {acceptor_id}")
        ...
        if len(self.promises) >= self.quorum:
            logging.debug(f"Proposer {self.id}: received quorum of promises, starting Phase 2")
            self.start_phase2()
    ...
    def start_phase2(self):
        ...
        accept_msg = encode_message("ACCEPT", self.current_ballot, self.id, self.current_value)
        
        logging.debug(f"Proposer {self.id}: starting Phase 2 with value {self.current_value}")
    ...
    def handle_accepted(self, msg):
        ...
        # Ignore if not for our current proposal
        if self.phase != "ACCEPT" or ballot != self.current_ballot or proposer_id != self.id:
            logging.debug(f"Proposer {self.id}: ignoring ACCEPTED (not current proposal)")
            return
        
        logging.debug(f"Proposer {self.id}: received ACCEPTED from acceptor {acceptor_id}")
        
        self.accepted_count += 1
        
        # Check if we have a quorum 
        if self.accepted_count >= self.quorum:
            logging.debug(f"Proposer {self.id}: quorum reached, sending DECISION to learners")
